...
# Threshold = 0.08
output = map_dfr(.x = seq(0.01, 0.07, 0.001), .f = rd_bw, threshold = 0.08)

colnames(output) = c("threshold", "bw", "estimate", "lower_bound", "upper_bound", "t_stat")
# Plotting Estimates

# Make it nice

ggplot(data = output, aes(x = bw, y = estimate)) +
  geom_line() +
  geom_line(aes(y = upper_bound)) +
  geom_line(aes(y = lower_bound))

# Threshold = 0.151
output2 = map_dfr(.x = seq(0.01, 0.07, 0.001), .f = rd_bw, threshold = 0.151)

colnames(output2) = c("threshold", "bw", "estimate", "lower_bound", "upper_bound")
# Plotting Estimates

# Make it nice

ggplot(data = output2, aes(x = bw, y = estimate)) +
  geom_line() +
  geom_line(aes(y = upper_bound)) +
  geom_line(aes(y = lower_bound))
